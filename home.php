...
$query = "SELECT COUNT(*) as num FROM tbl_transaction_details WHERE trans_for = 1 AND trans_type = 1 AND type = 2 AND status = 4 AND user_updated = 0";
$total_pages = mysqli_fetch_array(mysqli_query($mysqli, $query));
$withdrawal_requests = $total_pages['num'];

$qry_jobs = "SELECT SUM(if(tbl_jobs.job_status = 4, 1, 0)) AS active_jobs, SUM(if(tbl_jobs.job_status = 1, 1, 0)) AS completed_jobs, COUNT(*) as total_jobs FROM tbl_jobs WHERE status = 1";
$res_jobs = mysqli_fetch_array(mysqli_query($mysqli, $qry_jobs));
$active_jobs = $res_jobs['active_jobs'];
$completed_jobs = $res_jobs['completed_jobs'];
$total_jobs = $res_jobs['total_jobs'];

$qry_users = "SELECT SUM(if(tbl_users.user_type = 1, 1, 0)) AS num_seekers, SUM(if(tbl_users.user_type = 2, 1, 0)) AS num_providers  FROM tbl_users WHERE id <> 0 AND status = 2";
$total_user = mysqli_fetch_array(mysqli_query($mysqli, $qry_users));
$total_user_seeker = $total_user['num_seekers'];
$total_user_providers = $total_user['num_providers'];
...
for ($mon = 1; $mon <= 12; $mon++) {
    ...
    if (isset($_GET['filterByYear'])) {
        ...
        $sql_user = "SELECT `id` FROM tbl_users WHERE `register_date` <> 0  AND id <> 0 AND DATE_FORMAT(FROM_UNIXTIME(`register_date`), '%c') = '$mon' AND DATE_FORMAT(FROM_UNIXTIME(`register_date`), '%Y') = '$year' AND status = 2";
        $totalcount = mysqli_num_rows(mysqli_query($mysqli, $sql_user));
    } else {
        $month = date('M', mktime(0, 0, 0, $mon, 1, date('Y')));
        $sql_user = "SELECT `id` FROM tbl_users WHERE `register_date` <> 0  AND id <> 0 AND DATE_FORMAT(FROM_UNIXTIME(`register_date`), '%c') = '$mon' AND status = 2";
        $totalcount = mysqli_num_rows(mysqli_query($mysqli, $sql_user));
    }
...
<div class="row">
    <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
        <a href="manage_users.php?user_type=1" class="card card-banner card-orange-light">
            <div class="card-body"> <i class="icon fa fa-users fa-4x"></i>
                <div class="content">
                    <div class="title">Total Seekers</div>
                    <div class="value"><span class="sign"></span><?php echo $total_user_seeker; ?></div>
                </div>
            </div>
        </a>
    </div>
    ...
    <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
        <a href="manage_users.php?user_type=2" class="card card-banner card-pink-light">
            <div class="card-body"> <i class="icon fa fa-users fa-4x"></i>
                <div class="content">
                    <div class="title">Total Providers</div>
                    <div class="value"><span class="sign"></span><?php echo $total_user_providers; ?></div>
                </div>
            </div>
        </a>
    </div>
    ...
    <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
        <a href="manage_active_jobs.php" class="card card-banner card-yellow-light">
            <div class="card-body"> <i class="icon fa fa-black-tie fa-4x"></i>
                <div class="content">
                    <div class="title">Active Jobs</div>
                    <div class="value"><span class="sign"></span><?=$active_jobs; ?></div>
                </div>
            </div>
        </a>
    </div>

    <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12"> <a href="manage_jobs.php" class="card card-banner card-blue-light">
    ...
    <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
        <a href="manage_jobs.php" class="card card-banner card-alicerose-light">
            <div class="card-body"> <i class="icon fa fa-list fa-4x"></i>
                <div class="content">
                    <div class="title">Completed jobs</div>
                    <div class="value"><span class="sign"></span><?php echo $completed_jobs; ?></div>
                </div>
            </div>
        </a>
    </div>

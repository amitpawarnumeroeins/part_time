<?php 
	...
	define("PACKAGE_NAME",$settings_details['ios_bundle_identifier']);
   ...
   function get_job_info($job_id,$field_name) 
	 {
	 	...
	 	$qry_job="SELECT * FROM tbl_jobs WHERE tbl_jobs.`id`='".$job_id."'";
 ...
 $get_method = checkSignSalt($_POST['data']);	 
  ...
  if($get_method['method_name']=="get_home")
  ...
  else if($get_method['method_name']=="get_category")
 	{
 	    if(isset($get_method['page']))
 	     
 	      {
 	        ...
 	        $query_rec = "SELECT COUNT(*) as num FROM tbl_category WHERE tbl_category.`status`= 1";
    		...
    		$query="SELECT `cid`,`category_name`,`category_image` FROM tbl_category WHERE tbl_category.`status`=1 ORDER BY tbl_category.".$cat_order." LIMIT $limit, $page_limit";
 	    ...
 	    else
 	    {
    		...
    		$query="SELECT `cid`,`category_name`,`category_image` FROM tbl_category WHERE tbl_category.`status`=1 ORDER BY tbl_category.".$cat_order."";
    		$sql = mysqli_query($mysqli,$query)or die(mysqli_error($mysqli));     
    ...
    $query="SELECT * FROM tbl_users
             ...
             WHERE tbl_apply.`job_id`=".$get_method['apply_job_id']." 
    ...
    while($data = mysqli_fetch_assoc($sql))
    {
 		...
 		if($_FILES['job_image']['name']!="")
     {
      ...
      else
      {
		...
		$query="SELECT * FROM tbl_company  WHERE tbl_company.`user_id`='$user_id'
				ORDER BY tbl_company.`id` DESC";
		$sql = mysqli_query($mysqli,$query) or die(mysqli_error($mysqli));
	...
	{
	    //app details
	    $qry_user = "SELECT * FROM tbl_users WHERE tbl_users.`id` = '".$get_method['user_id']."'";
		...
		$query="SELECT * FROM tbl_settings WHERE `id`='1'";
		$sql = mysqli_query($mysqli,$query)or die(mysqli_error($mysqli));
		$row['ios_bundle_identifier'] = $settings_details['ios_bundle_identifier'];
